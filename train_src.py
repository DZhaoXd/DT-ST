...
def train(cfg, local_rank, distributed):
    ...
    if APEX_AVAILABLE:
        [feature_extractor, classifier], [optimizer_fea,optimizer_cls] = amp.initialize(
           [feature_extractor, classifier], [optimizer_fea,optimizer_cls], opt_level="O2", keep_batchnorm_fp32=True,
             loss_scale="dynamic"
        )  
    ...
    for i, (src_input, src_label, _) in enumerate(train_loader):
        ...
        pred = classifier(feature_extractor(src_input)[1], size)
        #pred = classifier(feature_extractor(src_input), size)

        loss = criterion(pred, src_label)
        # loss.backward()
        amp_backward(loss, [optimizer_fea, optimizer_cls])
...
def run_test(cfg, model, local_rank, distributed):
    ...
    with torch.no_grad():
        for i, (x, y, _) in enumerate(test_loader):
            ...
            size = y.shape[-2:]
            pred = classifier(feature_extractor(x)[1])
